...
class Domain:
    ...
    def proj_ball(self, X, center, radius):
        ...
        X_out[:, Norms > radius] = radius * X_out[:, Norms > radius] / Norms[Norms > radius]
    ...
    def proj(self, X):
        ...
        if self.dom_type == 'Box':
            ...
            center_domain = self.sizes[0]
            radius_domain = self.sizes[1]
            d = self.sizes[2]
...
def heuristics_ball(X, x_alpha, Rad):
    ...
    X_proj[:, where] = Rad * max_norms * X_proj[:, where] / np.repeat(norms[np.newaxis, where], d, axis=0)
...
def cbo(dt, sig, lam, alpha, X0, d, N, x_opt, maxit,
    ...
    while it < maxit:
        ...
        # plotting
        if it % show_every == 0 and it > show_every and Plot:
            show.plot_illustration(X, x_alpha, Box, it, 1e-2,
                                   Function='Rastriagin', Save=False)
